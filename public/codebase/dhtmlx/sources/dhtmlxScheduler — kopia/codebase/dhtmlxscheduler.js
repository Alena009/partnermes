			...
			if (this._drag_mode) {
				...
				if (this._is_pos_changed(this._drag_pos, t)) {
					...
					if (scheduler.matrix && (o = scheduler.matrix[scheduler._mode]), o = o || {
						...
						r = scheduler._correct_drag_end_date(i, this._drag_event._event_length)) : r = l.end_date.valueOf() - (l.start_date.valueOf() - i);
					else {
					...
					if ("move" == this._drag_mode && scheduler.config.limit_drag_out && (+v < +scheduler._min_date || +r > +scheduler._max_date)) {
						if (+l.start_date < +scheduler._min_date || +l.end_date > +scheduler._max_date) v = new Date(l.start_date), r = new Date(l.end_date);
						else {
			...
			if (this._configure(scheduler.config, scheduler._skin_settings, r), this._configure(scheduler.xy, scheduler._skin_xy, r),
				...
				d && (d.padding = 14), scheduler.templates.event_bar_date = function (e, t, i) {
					...
					if (scheduler.templates.month_day = function (i) {
							if ("month" == this._mode) {
								var r = e(i);
								return 1 == i.getDate() && (r = scheduler.locale.date.month_full[i.getMonth()] + " " + r), +i == +scheduler.date.date_part(this._currentDate()) && (r = scheduler.locale.labels.dhx_cal_today_button + " " + r),
									r
							}

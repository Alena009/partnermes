	...
	e._temp_matrix_scope = function () {
		e.matrix = {}, e._merge = function (e, t) {
			...
			e.render_data = function (n, i) {
					...
					if (i && !t.show_unassigned && "cell" != t.render)
						for (var r = 0; r < n.length; r++)
							this.clear_event(n[r]), this.render_timeline_event.call(this.matrix[this._mode], n[r], !0);
					else
						e._renderMatrix.call(t, !0, !0)
				}, e.matrix[t.name] =
				t, e.templates[t.name + "_cell_value"] = function (e) {
					return e ? e.length : ""
				...
				e.templates[t.name + "_date"] = function (t, a) {
					return t.getDay() ==
						...
						t.getDay() !=
						a.getDay() &&
						864e5 > a - t ? e.templates.day_date(t) + " &ndash; " + e.templates.day_date(a) :
					...
					if ("minute" == t.x_unit || "hour" == t.x_unit) {
						var o = i;
						"hour" == t.x_unit && (o *= 60), o % 1440 || (i = o / 1440,
							r = "day")
					}
					...
					if ("minute" == t.x_unit || "hour" == t.x_unit) {
						...
						if (d * o % 1440)
							if (+e.date.date_part(new Date(a)) == +e.date.date_part(new Date(r)))
								t.x_start += n * o;
							else {
								var l = 1440 / (o * d) - 1,
									s = Math.round(l * o);
			...
			e["mouse_" + t.name] = function (a) {
				...
				if (a.x = 0, a.force_redraw = !0, a.custom = !0, "move" == this._drag_mode && this._drag_id && this._drag_event) {
					var n = this.getEvent(this._drag_id),
						r = this._drag_event;
					if (a._ignores = this._ignores_detected || t._start_correction || t._end_correction,
						void 0 === r._move_delta && (r._move_delta = (n.start_date - i) / 6e4, this.config.preserve_length && a._ignores && (r._move_delta = this._get_real_event_length(n.start_date, i, t), r._event_length = this._get_real_event_length(n.start_date, n.end_date, t))), this.config.preserve_length && a._ignores) {
						var o = (r._event_length, this._get_fictional_event_length(i, r._move_delta, t, !0));
						i = new Date(i - o)
					} else
						i = e.date.add(i, r._move_delta, "minute")
				}
				if ("resize" == this._drag_mode && n && (this.config.timeline_swap_resize && this._drag_id && (this._drag_from_start && +i > +n.end_date ? this._drag_from_start = !1 : !this._drag_from_start && +i < +n.start_date && (this._drag_from_start = !0)), a.resize_from_start = this._drag_from_start, !this.config.timeline_swap_resize && this._drag_id && this._drag_from_start && +i >= +e.date.add(n.end_date, -e.config.time_step, "minute") && (i = e.date.add(n.end_date, -e.config.time_step, "minute"))), t.round_position)
					switch (this._drag_mode) {
						case "move":
							this.config.preserve_length || (i = e._timeline_get_rounded_date.call(t, i, !1), "day" == t.x_unit && (a.custom = !1));
							break;
						case "resize":
							this._drag_event && ((null === this._drag_event._resize_from_start ||
									void 0 === this._drag_event._resize_from_start) && (this._drag_event._resize_from_start = a.resize_from_start), a.resize_from_start = this._drag_event._resize_from_start,
								i = e._timeline_get_rounded_date.call(t, i, !this._drag_event._resize_from_start))
					}
			...
			if ("cell" == t.render)
				...
				for (var n = e.get_visible_events(),
						i = t.order,
						r = 0; r < n.length; r++) {
					var o = n[r],
						d = o[t.y_property],
			...
			e._rendered = [];
			for (var a = t.getElementsByTagName("DIV"),
					n = 0; n < a.length; n++)
				a[n].getAttribute("event_id") && e._rendered.push(a[n])
			...
			for (var t = e.get_visible_events(),
					a = [],
					n = 0; n < this.y_unit.length; n++)
				a[n] = [];
			var i;
			a[i] || (a[i] = []);
			for (var n = 0; n < t.length; n++) {
				i = this.order[t[n][this.y_property]];
				for (var r = 0; this._trace_x[r + 1] && t[n].start_date >= this._trace_x[r + 1];)
					r++;
			...
			if (s > 0) {
				...
				o ? +l > +u && a && (d = e._cols[_]) : (s = l - u, n.first_hour || n.last_hour ? (s -= n._start_correction, 0 > s && (s = 0),
					d = Math.round(s / r), d > e._cols[_] && (d = e._cols[_])) : d = Math.round(s / r))
			}
			...
			if (e._ignores_detected)
				for (var a,
					...
					for (r = t[o],
						...
						n = e._get_date_index(this, r.end_date); n > a;) {
						if (!e._ignores[a]) {
							i = !0;
							break
						}
			...
			if (t && "cell" != this.render) {
				e._timeline_skip_ignored.call(this, t), t.sort(this.sort ||
					function (e, t) {
					...
					if (n.length && (n[n.length - 1]._inner = !0), !s)
						if (n.length)
							if (n.length <= n[n.length - 1]._sorder) {
								...
								for (var v = n[0]._sorder,
										m = 1; m < n.length; m++)
									n[m]._sorder > v && (v = n[m]._sorder);
				...
				for (var g = 0; g < t.length; g++)
					t[g]._count = n.max_count;
				for (var f = 0; i > f; f++)
					a += e.render_timeline_event.call(this, t[f], !1)
			}
			return a
		}, e._timeline_y_scale = function (t) {
			var a = "<table style='table-layout:fixed;' cellspacing='0' cellpadding='0'>";
			e._load_mode && e._load();
			for (var n = e._prepare_timeline_events(this),
					i = 0,
					r = 0; r < e._cols.length; r++)
				i += e._cols[r];
			var o = new Date,
				d = e._cols.length - e._ignores_detected;
			o = (e.date.add(o, this.x_step * d, this.x_unit) - o - (this._start_correction + this._end_correction) * d) / i, this._step =
				o, this._summ =
				i;
			...
			for (var r = 0; r < this.y_unit.length; r++) {
				...
				if (a += "<tr class='" + _.tr_className + "' style='" + _.style_height + "'><td class='" + _.td_className + "' style='" + _.style_width + " height:" + (_.height - 1) + "px;' " + e._waiAria.label(_.td_content) + ">" + _.td_content + "</td>", "cell" == this.render)
					for (var h = 0; h < e._cols.length; h++)
						a += e._ignores[h] ? "<td></td>" : "<td class='dhx_matrix_cell " + e.templates[this.name + "_cell_class"](n[r][h], this._trace_x[h], this.y_unit[r]) + "' style='width:" + e._cols[h] + "px'><div style='width:auto'>" + e.templates[this.name + "_cell_value"](n[r][h], this._trace_x[h], this.y_unit[r]) + "</div></td>";
				else {
					...
					for (var h = 0; h < e._cols.length; h++)
						a += e._ignores[h] ? "<td></td>" : "<td class='dhx_matrix_cell " + e.templates[this.name + "_cell_class"](n[r], this._trace_x[h], this.y_unit[r]) + "' style='width:" + e._cols[h] + "px'></td>";
					a += "</table>", a += "</div></td>"
				}
				a += "</tr>", s.push(_)
			}
			a += "</table>", this._matrix =
				n, t.innerHTML =
				a, e._populate_timeline_rendered(t), this._scales = {};
			for (var p = t.firstChild.rows,
					v = null,
					r = 0,
					m = s.length; m > r; r++) {
			...
			for (var i = (this.x_size + (t ? e._ignores_detected : 0), 0),
					r = 0; + n > +a;)
				if (this._trace_x[r] = new Date(a), "month" == this.x_unit && e.date[this.x_unit + "_start"] && (a = e.date[this.x_unit + "_start"](new Date(a))),
					a = e.date.add(a, this.x_step, this.x_unit), e.date[this.x_unit + "_start"] && (a = e.date[this.x_unit + "_start"](a)), e._ignores[r] || i++, r++, t)
			...
			for (var d = e._min_date_timeline = e._min_date,
					l = e.config.preserve_scale_length,
					s = e._timeline_x_dates.call(this, l),
					_ = s.displayed,
					c = s.total,
					u = 0; c > u; u++)
				e._ignores[u] ? (e._cols[u] = 0, _++) : e._cols[u] = Math.floor(i / (_ - u)), i -= e._cols[u], r[u + 1] = r[u] + e._cols[u];
			if (t.innerHTML = "<div></div>", this.second_scale) {
				for (var h = this.second_scale.x_unit,
						p = [this._trace_x[0]],
						v = [],
						m = [this.dx, this.dx],
						g = 0,
						f = 0; f < this._trace_x.length; f++) {
				...
				for (var D = 0; D < p.length; D++) {
					...
					S.className = "dhx_scale_bar dhx_second_scale_bar" + (N ? " " + N : ""), e.set_xy(S, v[D] - 1, n - 3, m[D], 0), S.innerHTML = e.templates[this.name + "_second_scale_date"](E), k.appendChild(S)
				}
			}
			e.xy.scale_height = n,
				t = t.lastChild;
			...
			if ("cell" == t.render) {
				...
				e._tooltip.style.display = "", e._tooltip.style.top = "0px", document.body.offsetWidth - n.left - e._tooltip.offsetWidth < 0 ? e._tooltip.style.left = n.left - e._tooltip.offsetWidth + "px" : e._tooltip.style.left = n.left + a.src.offsetWidth + "px", e._tooltip.date =
					i, e._tooltip.innerHTML =
					d, document.body.offsetHeight - n.top - e._tooltip.offsetHeight < 0 ? e._tooltip.style.top = n.top - e._tooltip.offsetHeight + a.src.offsetHeight + "px" : e._tooltip.style.top = n.top + "px"
			}
			...
			for (var a = t.parentNode.childNodes,
					n = -1,
					i = 0; i < a.length; i++)
				if (a[i] == t) {
					n = i;
			...
			t = t || event;
			for (var a = t.target ? t.target : t.srcElement,
					n = {},
					i = e.matrix[e._mode],
					r = e.getActionData(t),
					o = e._ignores,
					d = 0,
					l = 0; l < i._trace_x.length - 1 && !(+r.date < i._trace_x[l + 1]); l++)
				o[l] || d++;
		...
		var a = e.dblclick_dhx_marked_timespan ||
			function () {};
		e.dblclick_dhx_marked_timespan = function (t) {
			...
			return e && (e._move_delta =
				void 0), n && (n._move_delta =
				void 0), !0
		...
		e._render_marked_timespan = function (t, a, i, r, o) {
			...
			if (e.matrix && e.matrix[e._mode]) {
				...
				if (i)
					...
					var u = d.order;
					if (c)
						u.hasOwnProperty(c) && (s.push(c), _.push(d._scales[c]));
					else if (d._scales)
						for (var h in u) u.hasOwnProperty(h) && (s.push(h), _.push(d._scales[h]))
				}
				var r = r ? new Date(r) : e._min_date,
					o = o ? new Date(o) : e._max_date;
				if (r.valueOf() < e._min_date.valueOf() && (r = new Date(e._min_date)), o.valueOf() > e._max_date.valueOf() && (o = new Date(e._max_date)), !d._trace_x)
					return;
				for (var p = 0; p < d._trace_x.length && !e._is_column_visible(d._trace_x[p]); p++);
				if (p == d._trace_x.length)
					return;
				var v = [];
				if (t.days > 6) {
					...
					v.push.apply(v, e._get_dates_by_index(t.days));
				for (var g = t.zones,
						f = e._get_css_classes_by_config(t),
						b = 0; b < s.length; b++) {
					a = _[b],
						i = s[b];
		...
		e._on_scale_add_marker = function (t, a) {
			if (e.matrix && e.matrix[e._mode]) {
				...
				if (i && e.matrix && e.matrix[e._mode])
					for (var r = e._mode,
							...
							l = e._max_date,
							s = i.global,
							_ = e.date.date_part(new Date(d)); l > _; _ = e.date.add(_, 1, "day")) {
						var c = +_,
							u = _.getDay(),
							h = [],
							p = s[c] || s[u];
						...
						for (var g = 0; g < h.length; g++) {
							var f = h[g],
								b = f.days;
							7 > b ? (b = c, n.push.apply(n, e._render_marked_timespan(f, t, a, _, e.date.add(_, 1, "day"))),
								b =
								u) : n.push.apply(n, e._render_marked_timespan(f, t, a, _, e.date.add(_, 1, "day")))
						}
					}
				return n
			}
			...
			for (var a = 0,
					n = e._trace_x; a < n.length - 1 && +t >= +n[a + 1];)
				a++;
			return a
			...
			if (0 === l && this._ignores[0])
				...
				for (l = this._cols.length - 1,
					n = 0; this._ignores[l];)
					l--;
				l++
			}
			var c;
			if (l >= r._trace_x.length)
				c = e.date.add(r._trace_x[r._trace_x.length - 1], r.x_step, r.x_unit), r._end_correction && (c = new Date(c - r._end_correction));
			else {
